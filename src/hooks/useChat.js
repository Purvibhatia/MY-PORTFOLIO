...
function mockBrain(history, userMsg) {
  ...
  if (/(resume|cv|pdf)/.test(text)) return "Résumé: /Purvi_Bhatia_Resume.pdf";
  if (/(email|contact|hire|reach)/.test(text))
    return "Email me at purvisophia7@gmail.com — I usually reply ASAP.";
  if (/(linkedin|github)/.test(text))
    return "LinkedIn: linkedin.com/in/purvi-bhatia-b55324248 · GitHub: github.com/Purvibhatia";
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content:
        "Hey, I’m Purvi — Purvi’s portfolio assistant. Ask me about About, Skills, Projects, Experience, Education, Contact, or availability.",
    },
  ]);
